...
async function fileIndex(query = "") {
  ...
  const results = trimmedQuery
    ? searchFileIndex(index, trimmedQuery).map(({ metadata, ...file }) => file)
    : [];
...
<body>
  <main>
    ...
    <div class="grid">
      ...
      <section><h2>自动同步时间</h2><div class="times"><label>时间 1<input id="time1" type="time"></label><label>时间 2<input id="time2" type="time"></label><button id="save">保存时间</button></div><div id="schedule" class="meta"></div></section>
      <section class="wide"><h2>运行日志</h2><pre id="log">暂无日志</pre></section>
      <section class="wide file-index"><div class="file-toolbar"><div class="file-actions"><h2>SharePoint 文件索引</h2><div class="file-controls"><button id="file-toggle" class="secondary">全部收起</button><span id="file-stats" class="meta">统计读取中</span></div></div><input id="file-search" type="search" placeholder="按文件名模糊查询"></div><div id="file-meta" class="meta">读取中</div><div id="file-results" class="file-results"></div><div id="file-tree" class="file-tree">读取中</div></section>
    </div>
  </main>
  <script>
    ...
    async function load() {
      ...
      $('schedule').textContent = '当前计划：' + (s.schedule || []).join('、');
    ...
    function renderTree(nodes) {
      return (nodes || []).map((node) => node.type === 'folder'
        ? '<details open><summary>📁 ' + escapeHtml(node.name) + ' <span class="meta">(' + fileCount(node) + ')</span></summary><div class="tree-children">' + renderTree(node.children) + '</div></details>'
        : '<div class="file-node"><span>📄</span><span class="name">' + escapeHtml(node.name) + '</span></div>').join('');
    }
    function renderResults(results) {
      return (results || []).map((file) => '<div class="file-result"><span class="name">📄 ' + escapeHtml(file.name) + '</span><span class="path">' + escapeHtml(file.directory || '(根目录)') + '</span><button class="copy-path" data-copy-path="' + escapeHtml(file.path) + '">复制路径</button></div>').join('');
    }
    ...
    async function loadFiles() {
      ...
      $('file-meta').textContent = query ? '匹配 ' + data.count + ' 个文件' : '共 ' + data.count + ' 个文件 · 更新于 ' + (data.updatedAt ? fmt(data.updatedAt) : '-');
      ...
      $('file-stats').textContent = '文件夹 ' + (data.stats?.folderCount || 0) + ' · 文件 ' + (data.stats?.fileCount || 0) + (extensionStats ? ' · ' + extensionStats : '');
    ...
    $('file-results').onclick = async (event) => { const button = event.target.closest('[data-copy-path]'); if (!button) return; await navigator.clipboard.writeText(button.dataset.copyPath); button.textContent = '已复制'; setTimeout(() => { button.textContent = '复制路径'; }, 1200); };
  </script>
</body>
</html>`;
